...
void print_usage(const char *program_name) { //定义一个函数来打印程序的使用说明，接受一个字符串参数 program_name，表示程序的名称，函数会输出如何正确使用程序以及可用的命令行选项和参数的详细信息
    ...
    printf("\n");
    printf("Example: %s -f sample.txt -t 10 -p -s -o result.html --html\n", program_name); // 输出一个示例命令行，展示如何使用程序来分析 sample.txt 文件，显示前10个单词的频率和百分比，启用停用词过滤，并将结果写入 result.html 文件中，以 HTML 格式输出
}
...
CliConfig parse_cli(int argc, char *argv[]) { //定义一个函数来解析命令行参数，接受两个参数：argc 表示参数的数量，argv 是一个字符串数组，包含了所有的命令行参数，函数会根据传入的参数设置配置结构体中的相应字段，并返回一个填充好的 CliConfig 结构体实例
    ...
    /* v2.0 新字段默认值 */
    cfg.case_sensitive = 0;  /* 默认不区分大小写 */
    cfg.use_stopwords = 0;   /* 默认不启用停用词过滤 */
    cfg.sort_mode = 0;      /* 0 = freq */
    cfg.html_output = 0;   /* 默认文本输出 */
    ...
    for (int i = 1; i < argc; i++) { //从 argv 数组的第二个元素开始循环遍历命令行参数，通过比较每个参数与预定义的选项来设置配置结构体中的相应字段值
        ...
        /* v2.0 新增 */
        else if (strcmp(argv[i], "-c") == 0) { //如果当前参数是 -c，表示用户希望启用大小写敏感的单词计数功能
            cfg.case_sensitive = 1; //将配置结构体中的 case_sensitive 字段设置为1
        }
        else if (strcmp(argv[i], "-s") == 0) { //如果当前参数是 -s，表示用户希望启用内置的停用词过滤功能
            cfg.use_stopwords = 1;
        }
        else if (strcmp(argv[i], "--sort") == 0) { //如果当前参数是 --sort，表示用户希望指定单词频率统计结果的排序方式，可以选择按频率降序排序（默认）或者按字母顺序排序
            if (i + 1 < argc) { //确保有下一个参数作为排序方式
                if (strcmp(argv[i + 1], "alpha") == 0) //如果下一个参数是 "alpha"，表示用户希望按字母顺序排序
                    cfg.sort_mode = 1;
                else
                    cfg.sort_mode = 0;  /* "freq" 或非法值 → 默认降序 */
                i++;
            } else {
                fprintf(stderr, "Error: --sort requires 'freq' or 'alpha'.\n"); /* 如果缺少排序方式参数，或者提供的参数不是 "freq" 或 "alpha"，则输出错误信息并设置错误标志 */
                cfg.has_error = 1;
                return cfg;
            }
        }
        else if (strcmp(argv[i], "--html") == 0) { //如果当前参数是 --html，表示用户希望将输出结果以 HTML 格式生成一个报告文件，而不是默认的纯文本格式
            cfg.html_output = 1;
        }
        else {
            /* 未知参数，忽略 */
